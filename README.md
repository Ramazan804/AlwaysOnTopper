...
Make sure that .NET RUNTIME is installed. Just run **AlwaysOnTopper.exe**. It's recommended to start it on login using Task Scheduler for daily use. For it to take effect in programs with administrator privileges, the application must also be granted administrator privileges.
